...
traffic_light::traffic_light(int pin1, int pin2, int pin3, int pin4, int pin5, int pin6){
  pinMode(pin1, OUTPUT);
  pinMode(pin2, OUTPUT);
  pinMode(pin3, OUTPUT);
  pinMode(pin4, OUTPUT);
  pinMode(pin5, OUTPUT);
  pinMode(pin6, INPUT);
  led_red = pin1; // the red LED is connected to Pin 0 of the Arduino
  led_yellow = pin2; // the yellow LED is connected to Pin 1 of the Arduino
  led_green = pin3; // the green LED is connected to Pin 2 of the Arduino
  led_redp = pin4;
  led_greenp = pin5;
  puls = pin6;
...
float traffic_light::residuo(float i, float j){
  float residue;
  residue= float(i/j)-float(int(i/j));
  return residue;
}
void traffic_light::esperar(int g){
